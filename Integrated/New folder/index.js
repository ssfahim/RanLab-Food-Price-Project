...
function openMenu(menuType) {
    ...
    switch (menuType) {
        case "ChristmasMenu":
            ...
            `;
            var measurement = `
            3 cups(360 grams),1 1/2 cup(180 grams),1/2 cup(60 ml),1 cup(120 grams),1/2 cup(60 ml)
            `;
            populateMenu(menuContent, lst, measurement,category);
            closeMenu();
            break;
        case "BakingCake":
            var lst = `
            Wheat flour,White sugar,Vegetable oil,Milk,Butter
            `;
            var measurement = `
            3 cups(360 grams),1 1/2 cup(180 grams),1/2 cup(118.5 ml),1 cup(120 grams),1/2 cup(118.5 ml)
            `;
            populateMenu(menuContent, lst, measurement,category);
            closeMenu();
            break;
        ...
        case "BakingPie":
            ...
            `;
            var measurement = `
            6 unit(410 grams),1/4 cup(60 grams),1 1/4 cup(150 grams),1 tablespoon(14.78 ml),1/2 cup(118.5 ml)
            `;
            populateMenu(menuContent, lst, measurement,category);
            closeMenu();
            break;
...
function populateMenu(container,lst, measurement,category) {
    // var category = document.getElementById('category').value;
    var menuContent = document.getElementById("selectedTablesd");
    ...
    var a = []; // in which columns the dates are located
    var p = []; // in which row the product is located
    ...
    switch(category){
        case 'menuPrice':
            var lst1 = []; // stores the actual measurements of each item of the recipe in grams
            var lst2 = []; // stores the actual measurements of each item of the recipe from the main Price Hike.csv file
            var actualUnit = []; // stores the measurements in grams by converting from kilograms
            var measure = measurement.trim().split(',');
            for(let i = 0; i<measure.length;i++){
                var measures = measure[i].trim().split('(');
                var measuress = measures[1].trim().split(' ');
                lst1.push(parseInt(measuress[0],10));
            }
            // console.log(lst1);
            for (var z = 0; z<p.length;z++){
                var product = rows[p[z]].split(',')[1].trim();
                lst2.push(product);
            }
            // console.log('lst2-->',lst2);
        
            for(let x = 0; x<lst2.length;x++){
                var pop = lst2[x].split(' ');
                // console.log(parseFloat(pop[0]));
                if (parseFloat(pop[0]) <= 6){
                    var cal = parseFloat(pop[0]) * 1000;
                    // console.log('var-->',cal)
                    actualUnit.push(cal);
                }
                else if(pop[0] == 'per'){
                    ...
                    var cal = 30;
                    console.log('unit-->',cal)
                    actualUnit.push(cal);
                }
                else{
                    // console.log('varrrrrrrrrrr-->',parseFloat(pop[0]));
                    actualUnit.push(parseFloat(pop[0]));
                }
            }
            // console.log('price-->',actualUnit)
            var c = measurement.trim().split(",");
            var lst3 = [];
            for(let g = 0; g<c.length;g++){
                lst3.push(c[g]);
            }
            var price = [];
            // console.log('Measurements----------------------->',lst3)
            var lst3Index = 0;
            for (var j = 0; j < p.length; j++) {
                ...
                for (var m = 0; m < lines.length; m++) {
                    ...
                    if (cells.length >= 2 && cells[0] === product) { 
                        ...
                        var cell = newRow.insertCell(-1);
                        // cell.innerHTML = cells[1];
                        for(let r =0; r<lst3.length;r++){
                            cell.innerHTML = lst3[lst3Index];
                            // console.log('index',lst3Index)
                            var lst2Index = lst3Index;
                            lst3Index++;
                            break;
                        }
                        for (var n = 0; n < a.length; n++) {
            ...
            for (var i = 0; i < totalPrice.length; i++) {
                ...
                // Initialize a variable to store the total of the current inner list
                var innerListTotal = 0;
                // Loop through each element in the inner list and add it to the total
                for (var j = 0; j < innerList.length; j++) {
                    innerListTotal += innerList[j];
                }
        ...
        case 'storePrice':
            ...
            for(let q = 0; q<p.length;q++){
                for(let t=0; t<a.length;t++){
                    var productRowColumn = rows[p[q]].split(',')[a[t]].trim();
                    totalPrice[t].push(productRowColumn++);
                    // console.log('pppppppppppppppppppppppppppppppppppppppppppppp', productColumn)
                }
            }
            // console.log(totalPrice);
            var tot = ["","<b>Total Price:</b> "];
    ...
    var lines = csvData.split("\n");
        for (var m = 0; m < lines.length; m++) {
            ...
            if (cells.length >= 2 && cells[0] === product) { 
                ...
                // console.log('pricesssssssssss', prices)
...
Dry or fresh pasta,500 grams,2.11,2.08,2.2,2.32,2.03,2.14,2.23,2.06,2.52,2.33,2.39,2.39,2.12,2.09,2.45,2.04,2.46,2.24,1.89,2.31,2.28,2.27,2.27,2.69,2.14,2.19,2.02,2.31,2.56,2.18,2.45,2.19,2.09,2.52,2.63,2.38,2.38,2.24,2.41,2.6,2.75,2.94,2.46,2.16,2.45,2.35,2.54,2.51,2.3,2.53,2.72,2.32,2.77,2.27,2.35,2.12,2.88,2.71,2.82,3.01,2.71,3.03,3.48,3.07,3.25,3.37,3.51,3.32,3.74,3.66,3,3.79,3.25,3.75,4.08,4.01,3.68,3.56,3.53,4.17,3.7,3.92,3.59
Brown rice,900 grams,4.81,4.59,4.81,4.87,4.93,5.06,4.91,5.05,4.91,4.67,4.78,4.84,4.62,4.85,4.99,4.84,4.9,4.89,5.08,5,4.94,4.76,4.75,4.53,4.85,4.77,4.71,4.84,4.89,4.6,4.85,4.7,4.62,4.86,5.17,5.21,5.48,5.26,5.4,5.47,5.63,5.59,5.49,5.06,5.21,5.53,5.41,5.72,5.2,5.52,5.48,5.07,5.17,5.13,5.52,5.24,5.07,5.11,5.38,5.16,5.18,5.43,5.33,4.99,5.05,5.41,5.75,5.83,5.82,6.01,6.09,5.8,5.61,6.07,5.93,5.62,5.89,5.99,5.64,5.77,5.5,6.13,5.46
White rice,2 kilogram,8.94,8.94,9.18,9.28,9.22,9.6,9.19,9.68,9.42,9.3,9.41,9.64,9.17,9.43,9.53,9.26,9.6,9.46,9.69,9.59,9.84,9.55,9.13,8.86,9.58,9.29,9.12,9.27,9.33,9.18,9.43,9.19,9.2,9.44,9.82,10.3,10.05,9.93,10.09,10.27,10.54,10.54,10.48,10.06,10.55,10.65,10.38,10.52,10.32,10.38,10.38,9.99,10.13,10.16,10.56,10.05,9.76,10.22,10.52,10.2,10.42,11.1,10.96,10.3,10.59,10.87,11.33,10.95,11.48,11.73,11.9,11.25,10.94,11.54,11.5,11.61,11.2,11.06,11.35,11.25,11.24,10.9,10.84
